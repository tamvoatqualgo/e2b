...
// authConfig will be populated at runtime with ECR credentials
...
func (r *Rootfs) pullDockerImage(ctx context.Context, tracer trace.Tracer) error {
	...
	//cmd := exec.CommandContext(childCtx, "aws", "ecr", "get-login-password", "--region", consts.AWSRegion)
	//password, err := cmd.Output()
	//if err != nil {
	//	errMsg := fmt.Errorf("error getting ECR auth token: %w", err)
	//	telemetry.ReportCriticalError(childCtx, errMsg)
	//	return errMsg
	//}

	// Set up auth config with ECR credentials
	//authConfig = registry.AuthConfig{
	//	Username: "AWS",
...
func (r *Rootfs) dockerTag() string {
	return fmt.Sprintf("%s.dkr.ecr.%s.amazonaws.com/%s/%s:%s", consts.AWSAccountID, consts.AWSRegion, consts.ECRRepository, r.env.TemplateId, r.env.BuildId)
}
