...
echo "Uploading envs from ${TARGET_DIR_NAME} to S3 bucket ${TEMPLATE_BUCKET_NAME}"

COMMAND="aws s3 cp --quiet --cache-control no-cache,max-age=0"
...
for template_id in $(ls ${TARGET_DIR_NAME}); do
  ...
  # Upload env to S3 with aws s3 cp, copy only "memfile", "rootfs.ext4" and "snapfile" from the dir
